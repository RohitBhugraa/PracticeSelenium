...
public class Product extends PageObject {
	...
	public List<WebElementFacade> getListOfProducts() {
		...
		String productListXpath = "//span[@class= 'product-name__item product-name__item--name']";
...
//		}
		...
		for (int i=0; i<list.size(); i++) {
			...
			if (list.get(i).getText().equals(search)) {
				...
				System.out.println("Selected " + getDriver().findElement(By.name(search)).isSelected());
				break;
			}
		}
